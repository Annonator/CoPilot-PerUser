...
func (c *BillingClient) GetAICreditUsage(ctx context.Context, usageRequest AICreditUsageRequest) (*AICreditUsageReport, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("parse GitHub billing URL: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("create GitHub billing request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("request GitHub billing usage: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		return nil, fmt.Errorf("GitHub billing usage status %d", resp.StatusCode)
	}

	var report AICreditUsageReport
	if err := json.NewDecoder(resp.Body).Decode(&report); err != nil {
		return nil, fmt.Errorf("decode GitHub billing usage: %w", err)
	}
	return &report, nil
}
